...
export class IntentTracker {
  private intentPatterns: Map<RegExp, CommandIntent> = new Map();
  private commandHistory: Array<{ command: string; intent: CommandIntent; timestamp: Date }> = [];
  ...
  private enhanceIntentWithContext(baseIntent: CommandIntent, aiContext?: string): CommandIntent {
    if (!aiContext) return baseIntent;
  ...
  private analyzeHeuristically(command: string, aiContext?: string): CommandIntent {
  ...
  private recordIntent(command: string, intent: CommandIntent): void {
  ...
  getIntentSummary(): { categories: Record<string, number>; totalCommands: number } {
