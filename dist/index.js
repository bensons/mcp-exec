...
const mcp_logger_1 = require("./audit/mcp-logger");
...
const DEFAULT_CONFIG = {
    ...
    mcpLogging: {
        ...
        minLevel: process.env.MCP_EXEC_MCP_LOG_LEVEL || 'info',
...
const GetConfigurationSchema = zod_1.z.object({
    section: zod_1.z.enum(['security', 'logging', 'sessions', 'output', 'display', 'context', 'lifecycle', 'terminalViewer', 'all']).optional().describe('Configuration section to retrieve'),
});
const UpdateConfigurationSchema = zod_1.z.object({
    section: zod_1.z.enum(['security', 'logging', 'sessions', 'output', 'display', 'context', 'lifecycle', 'terminalViewer']).describe('Configuration section to update'),
    settings: zod_1.z.record(zod_1.z.any()).describe('Section-specific settings to update'),
});
const ResetConfigurationSchema = zod_1.z.object({
    section: zod_1.z.enum(['security', 'logging', 'sessions', 'output', 'display', 'context', 'lifecycle', 'terminalViewer', 'all']).optional().describe('Configuration section to reset'),
});
const ManageBlockedCommandsSchema = zod_1.z.object({
    action: zod_1.z.enum(['add', 'remove', 'list']).describe('Action to perform'),
    commands: zod_1.z.array(zod_1.z.string()).optional().describe('Commands to add or remove'),
});
const ManageAllowedDirectoriesSchema = zod_1.z.object({
    action: zod_1.z.enum(['add', 'remove', 'list']).describe('Action to perform'),
    directories: zod_1.z.array(zod_1.z.string()).optional().describe('Directories to add or remove'),
});
const UpdateResourceLimitsSchema = zod_1.z.object({
    maxMemoryUsage: zod_1.z.number().optional().describe('Maximum memory usage in MB'),
    maxFileSize: zod_1.z.number().optional().describe('Maximum file size in MB'),
    maxProcesses: zod_1.z.number().optional().describe('Maximum number of processes'),
});
const UpdateMcpLoggingSchema = zod_1.z.object({
    minLevel: zod_1.z.enum(['emergency', 'alert', 'critical', 'error', 'warning', 'notice', 'info', 'debug']).optional().describe('Minimum log level for MCP notifications'),
    rateLimitPerMinute: zod_1.z.number().optional().describe('Maximum messages per minute'),
    maxQueueSize: zod_1.z.number().optional().describe('Maximum queued messages'),
    includeContext: zod_1.z.boolean().optional().describe('Include context data in messages'),
});
const UpdateAuditLoggingSchema = zod_1.z.object({
    retention: zod_1.z.number().optional().describe('Log retention in days'),
    monitoringEnabled: zod_1.z.boolean().optional().describe('Enable monitoring alerts'),
    desktopNotifications: zod_1.z.boolean().optional().describe('Enable desktop notifications'),
    alertRetention: zod_1.z.number().optional().describe('Alert retention in days'),
    maxAlertsPerHour: zod_1.z.number().optional().describe('Maximum alerts per hour'),
});
...
const UpdateTerminalViewerSchema = zod_1.z.object({
    ...
    host: zod_1.z.string().optional().describe('Host for terminal viewer service'),
    enableAuth: zod_1.z.boolean().optional().describe('Enable authentication'),
    authToken: zod_1.z.string().optional().describe('Authentication token'),
    maxSessions: zod_1.z.number().optional().describe('Maximum terminal viewer sessions'),
    sessionTimeout: zod_1.z.number().optional().describe('Terminal session timeout in milliseconds'),
    bufferSize: zod_1.z.number().optional().describe('Terminal buffer size'),
});
const UpdateOutputFormattingSchema = zod_1.z.object({
    formatStructured: zod_1.z.boolean().optional().describe('Format output in structured format'),
    stripAnsi: zod_1.z.boolean().optional().describe('Strip ANSI escape codes'),
    enableAiOptimizations: zod_1.z.boolean().optional().describe('Enable AI-powered optimizations'),
    maxOutputLength: zod_1.z.number().optional().describe('Maximum output length in bytes'),
    summarizeVerbose: zod_1.z.boolean().optional().describe('Summarize verbose output'),
});
const UpdateDisplayOptionsSchema = zod_1.z.object({
    showCommandHeader: zod_1.z.boolean().optional().describe('Show command header information'),
    showExecutionTime: zod_1.z.boolean().optional().describe('Show execution time'),
    showExitCode: zod_1.z.boolean().optional().describe('Show exit code'),
    formatCodeBlocks: zod_1.z.boolean().optional().describe('Format code blocks'),
    includeMetadata: zod_1.z.boolean().optional().describe('Include metadata'),
    includeSuggestions: zod_1.z.boolean().optional().describe('Include suggestions'),
    useMarkdown: zod_1.z.boolean().optional().describe('Use Markdown formatting'),
    colorizeOutput: zod_1.z.boolean().optional().describe('Colorize output'),
});
const UpdateContextConfigSchema = zod_1.z.object({
    preserveWorkingDirectory: zod_1.z.boolean().optional().describe('Preserve working directory between commands'),
    sessionPersistence: zod_1.z.boolean().optional().describe('Enable session persistence'),
    maxHistorySize: zod_1.z.number().optional().describe('Maximum command history size'),
});
const UpdateLifecycleConfigSchema = zod_1.z.object({
    inactivityTimeout: zod_1.z.number().optional().describe('Inactivity timeout in milliseconds'),
    gracefulShutdownTimeout: zod_1.z.number().optional().describe('Graceful shutdown timeout in milliseconds'),
    enableHeartbeat: zod_1.z.boolean().optional().describe('Enable heartbeat monitoring'),
});
const GetConfigurationHistorySchema = zod_1.z.object({
    limit: zod_1.z.number().optional().describe('Number of configuration changes to retrieve'),
});
const RollbackConfigurationSchema = zod_1.z.object({
    changeId: zod_1.z.string().describe('Configuration change ID to rollback to'),
});
class MCPShellServer {
    ...
    constructor(config = {}) {
        ...
        this.terminalSessionManager = new terminal_session_manager_1.TerminalSessionManager(this.config.sessions, this.config.terminalViewer, (command, cwd) => this.assertCommandAllowed(command, 'terminal-session', {}, cwd));
    ...
    async assertCommandAllowed(command, source, extraContext = {}, cwd) {
        await (0, command_policy_1.assertCommandAllowed)(this.securityManager, command, this.auditLogger, { source, ...extraContext }, await this.getEffectiveCwd(cwd));
    }
    setupHandlers() {
                ...
                switch (name) {
                    case 'execute_command': {
                        ...
                        if (parsed.enableTerminalViewer) {
                            ...
                            try {
                                ...
                                await this.assertCommandAllowed(fullCommand, 'execute_command', { enableTerminalViewer: true }, parsed.cwd);
                    ...
                    case 'start_interactive_session': {
                        ...
                        try {
                            ...
                            await this.assertCommandAllowed((0, command_policy_1.buildFullCommand)(sessionCommand, parsed.args), 'start_interactive_session', {}, parsed.cwd);
                    ...
                    case 'start_terminal_session': {
                        ...
                        try {
                            if (parsed.command) {
                                await this.assertCommandAllowed((0, command_policy_1.buildFullCommand)(parsed.command, parsed.args), 'start_terminal_session', {}, parsed.cwd);
                            }
                            // Ensure terminal viewer service is available
                            if (!this.terminalViewerService) {
                                this.terminalViewerService = new viewer_service_1.TerminalViewerService(this.config.terminalViewer);
                    ...
                    case 'send_to_session': {
                        ...
                        try {
                            await this.assertCommandAllowed(parsed.input, 'send_to_session', { sessionId: parsed.sessionId }, this.terminalSessionManager?.getSession(parsed.sessionId)?.cwd);
                    ...
                    case 'update_configuration': {
                        ...
                        this.recordConfigurationChange(section, settings, previousValues);
                    ...
                    case 'reset_configuration': {
                        ...
                        for (const resetSection of resetSections) {
                            ...
                            if (resetSectionConfig) {
                                this.recordConfigurationChange(resetSection, resetSectionConfig, previousValues);
                            }
                    ...
                    case 'manage_blocked_commands': {
                        ...
                        switch (action) {
                            ...
                            case 'remove':
                                ...
                                this.config.security.blockedCommands = this.config.security.blockedCommands.filter(cmd => !commands.includes(cmd));
                                result = {
                                    action: 'remove',
                                    removed,
                                    totalBlocked: this.config.security.blockedCommands.length
                    ...
                    case 'manage_allowed_directories': {
                        ...
                        switch (action) {
                            ...
                            case 'remove':
                                ...
                                this.config.security.allowedDirectories = this.config.security.allowedDirectories.filter(dir => !directories.includes(dir));
                                result = {
                                    action: 'remove',
                                    removed,
                                    totalAllowed: this.config.security.allowedDirectories.length
                    ...
                    case 'rollback_configuration': {
                        ...
                        this.recordConfigurationChange(changeEntry.section, changeEntry.previousValues, JSON.parse(JSON.stringify(this.config[changeEntry.section])));
                        // Reinitialize components
...
Please start by enabling the terminal viewer service.`,
    ...
    recordConfigurationChange(section, changes, previousValues, user) {
