...
function resolvePath(target, baseDir, expandTilde = true) {
...
function tokenizeShell(command) {
    ...
    let expandable = [];
    let unquoted = [];
    const append = (character, canExpand, isUnquoted) => {
        value += character;
...
function expandShellWord(word, environment, cwd) {
    let expanded = '';
...
class SecurityManager {
    ...
    configurationBase;
    auditLogger;
    constructor(config, auditLogger, configurationBase = SERVER_STARTUP_CWD) {
        this.config = config;
    ...
    validateResolvedPath(resolvedPath, displayPath) {
    ...
    resolveFileReference(token, baseDir) {
        if (!/^file:/i.test(token)) {
