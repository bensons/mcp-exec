...
exports.normalizePathOperand = normalizePathOperand;
exports.parseBlockedCommandsEnvironment = parseBlockedCommandsEnvironment;
...
const WRAPPERS = {
    sudo: {
        ...
        longArgOptions: set('user', 'group', 'host', 'prompt', 'close-from', 'chdir', 'role', 'type', 'other-user', 'command-timeout', 'preserve-env', 'remove-timestamp'),
        longNoArgOptions: set('background', 'bell', 'edit', 'help', 'login', 'non-interactive', 'preserve-groups', 'reset-timestamp', 'stdin', 'validate', 'version'),
    },
    doas: {
        shortArgOptions: 'aCu',
    ...
    xargs: {
        ...
        longArgOptions: set('arg-file', 'eof', 'replace', 'max-lines', 'max-args', 'max-chars', 'max-procs', 'process-slot-var', 'delimiter'),
        longNoArgOptions: set('null', 'open-tty', 'interactive', 'verbose', 'no-run-if-empty', 'exit', 'show-limits', 'help', 'version'),
    },
};
 ...
 */
function parseCommand(command, platform = defaultPlatform(), depth = 0) {
    if (depth > 12) {
...
function tokenizeCommand(command, platform = defaultPlatform()) {
    return parseCommand(command, platform).subCommands;
}
function lexCommand(command, platform, depth) {
    const segments = [];
    const nested = [];
    let tokens = [];
    let current = '';
...
function findCommandSubstitution(command, start) {
    let nesting = 1;
    let quote = '';
...
function findBacktickEnd(command, start) {
    for (let i = start; i < command.length; i++) {
        if (command[i] === '\\') {
            i++;
        }
...
function stripControlSyntax(tokens) {
    const result = [...tokens];
    if (result.length === 0)
        return result;
    if (CONTROL_ONLY.has(result[0]))
        return [];
    if (DECLARATION_PREFIXES.has(result[0]))
        return [];
...
function toSubCommand(tokens, platform) {
    ...
    for (const token of argv.slice(1)) {
        ...
        if (!flagsDone && platform === 'win32' && /^\/[A-Za-z?]+(?::.*)?$/.test(token)) {
            ...
            flags.add(name);
            if (colon >= 0)
                flags.add(`${name}:${option.slice(colon + 1).toLowerCase()}`);
            continue;
        }
...
function stripWrappers(tokens, platform) {
    let i = 0;
    const wrappers = [];
...
function consumeWrapperOption(tokens, index, spec, wrapper) {
    ...
    for (let offset = 0; offset < cluster.length; offset++) {
        ...
        if (spec.shortNoArgOptions.includes(flag))
            continue;
...
function parseInterpreterPayload(sub, depth) {
 ...
 */
function matchesPattern(sub, pattern) {
    if (!pattern.argv0) {
        return pattern.wrappers.length === 1 && sub.wrappers.includes(pattern.wrappers[0]);
    }
    if (!commandNameMatches(sub.argv0, pattern.argv0))
        return false;
...
function commandNameMatches(actual, expected) {
    return actual === expected || actual.startsWith(`${expected}.`);
}
function operandMatches(actual, expected, platform) {
    const normalizedActual = normalizeValue(actual, platform);
    const normalizedExpected = normalizeValue(expected, platform);
    if (normalizedActual === normalizedExpected)
        return true;
    if (!looksLikePath(actual, platform) || !looksLikePath(expected, platform))
        return false;
...
function normalizePathOperand(value, platform = defaultPlatform()) {
    if (platform === 'win32') {
...
function normalizeFlag(value, platform) {
    return platform === 'win32' ? value.toLowerCase() : value;
}
function normalizeValue(value, platform) {
    return platform === 'win32' ? value.toLowerCase() : value;
}
function executableBasename(value, platform) {
    const basename = platform === 'win32' ? path.win32.basename(value) : path.basename(value);
    return basename.toLowerCase();
}
function isDynamicExecutable(value, platform) {
 ...
 */
function parseBlockedCommandsEnvironment(value) {
    const entries = [];
    let current = '';
    ...
    for (let i = 0; i < value.length; i++) {
        ...
        if (regexMode && c === ']' && inCharacterClass)
            inCharacterClass = false;
        if (regexMode && c === '{' && !inCharacterClass)
            quantifierDepth++;
        if (regexMode && c === '}' && quantifierDepth > 0 && !inCharacterClass)
            quantifierDepth--;
